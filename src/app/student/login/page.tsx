...
import { useLoginStudentMutation } from '@/app/store/api/auth/authApi';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      [phone.charAt(0).toLowerCase() === 's' ? 'ID' : 'phone']: phone,
      password
    };
    ...
    try {
      ...
      if (response?.accessToken) {
        ...
        setTimeout(() => {
          router.push('/student/pages/dashboard');
        }, 1000);
      }
    } catch (error: unknown) {
