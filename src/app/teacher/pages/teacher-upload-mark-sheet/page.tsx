...
import { useCreateBulkResultMutation, useGetStudentsForResultQuery } from '@/app/store/api/classes/resultApi';
...
const UploadMarkSheet = () => {
    ...
    const [createBulkResult, { isLoading: isSubmitting }] = useCreateBulkResultMutation();
    ...
    const subjects: Subject[] = useMemo(() => subjectsResponse?.data || [], [subjectsResponse?.data]);
    ...
    React.useEffect(() => {
        if (studentsResponse) {
            ...
            if (studentsResponse.data.length > 0 && filterSubject) {
                ...
                const studentData = studentsResponse.data.map((student: Student, index: number) => ({
                    SL: index + 1,
    ...
    const handleInputChange = (index: number, key: StudentDataKeys, value: string) => {
        const newData = [...data];
        ...
        if (key === 'MARKS') {
            ...
            const passMarks = subjects.find(s => s.id === filterSubject)?.passMarks || 33;
            newData[index]['STATUS'] = enteredMarks >= passMarks ? 'Passed' : 'Failed';
        } else {
            (newData[index][key] as string) = value;
        }
        ...
        reader.onload = async (loadEvent: ProgressEvent<FileReader>) => {
            try {
                ...
                const processedData = jsonData.map((row, index) => {
                    ...
                    const marks = typeof marksValue === 'number'
                        ? marksValue
                        : parseInt(marksValue.toString(), 10) || 0;

                    const totalMarksValue = row['TOTAL MARKS'] || '100';
                    const totalMarks = typeof totalMarksValue === 'number'
                        ? totalMarksValue
                ...
                if (filterExam && filterSubject) {
                    ...
                    try {
                        ...
                        console.error('Failed to submit results from uploaded file:', error);
                    }
                }
            } catch (error) {
                ...
                if (form) {
                    ...
                    newInput.onchange = (ev: Event) => {
                        handleFileUpload(ev as unknown as React.ChangeEvent<HTMLInputElement>);
                    };
    ...
    const submitResults = async () => {
        ...
        const studentsPayload = data.map(row => {
    ...
    const downloadPDF = () => {
        ...
        pdf.text(`Exam: ${examName}`, pageWidth / 2, y, { align: "center" });
        y += 20;
        pdf.text(`Class: ${className}`, pageWidth / 2, y, { align: "center" });
        y += 20;
        pdf.text(`Subject: ${subjectName}`, pageWidth / 2, y, { align: "center" });
        y += 20;
        ...
        const colWidths = [
            availableWidth * 0.1,  // SL
            availableWidth * 0.25, // STUDENT NAME
            availableWidth * 0.1,  // ROLL NO
            availableWidth * 0.1,  // TOTAL MARKS
            availableWidth * 0.1,  // MARKS
        ...
        data.forEach((row) => {
            ...
            rowData.forEach((data, i) => {
                ...
                // Center-align all cells except Student Name (left-aligned)
                const align = i === 1 ? "left" : "center";
                const padding = i === 1 ? 10 : 0;

                pdf.text(data, x + (align === "center" ? colWidths[i] / 2 : padding), tableY + 16, {
                    align,
        ...
        pdf.setFontSize(12);
        pdf.text("Teacher's Signature", pageWidth - margin - 100, footerY);
    ...
    const downloadTemplate = () => {
        const totalMarks = subjects.find(s => s.id === filterSubject)?.marks || 100;
            ...
            <div className="bg-white px-5">
                <Paper sx={{ mt: 2, p: 2 }}>
                    <Box sx={{
                        ...
                        <FormControl sx={{ py: 1 }} variant="outlined" size="small">
                            <InputLabel id="filter-exam-label">Exam</InputLabel>
                            <Select
                        ...
                        <FormControl sx={{ py: 1 }} variant="outlined" size="small">
                            ...
                            <Select
                                ...
                                value={filterSession || ''}
                                label="Session"
                                onChange={(e) => setFilterSession(Number(e.target.value) || null)}
                            >
                        ...
                        <Box sx={{
                            ...
                            <Box sx={{
                                ...
                                <div className='block md:hidden'>
                                    <SubmitButton disabled={isSubmitting} onClick={submitResults}>{isSubmitting ? (
                        ...
                        <div ref={tableRef}>
                            {filterSubject && searchParams && (
                                <Typography variant="h6" gutterBottom>
                                    ...
                                    {filterSubject && (
                                        <span> (Total Marks: {subjects.find(s => s.id === filterSubject)?.marks}, Pass Marks: {subjects.find(s => s.id === filterSubject)?.passMarks})</span>
                                    )}
                                </Typography>
                            )}
                            ...
                            {data.length > 0 ? (
                                <ReusableTable
                                    ...
                                    data={data.map((row, rowIndex) => ({
                                        ...
                                        'MARKS': (
                                            <TextField
                                                value={row['MARKS']}
                                                onChange={(e) => handleInputChange(rowIndex, 'MARKS', e.target.value)}
                                                size="small"
                                                type="number"
                                                inputProps={{
                                                    min: 0,
                                                ...
                                                error={parseInt(row['MARKS'] || '0') > row['TOTAL MARKS']}
                                                helperText={
                                ...
                                <Typography variant="body1" sx={{ p: 2 }}>
                                    Please select session, class, and subject, then click Search to view students.
                                </Typography>
                            )}
                        </div>
                    )}
