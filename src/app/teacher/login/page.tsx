...
import { useLoginTeacherMutation } from '@/app/store/api/auth/authApi';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      [phone.charAt(0).toLowerCase() === 't' ? 'ID' : 'phone']: phone,
      password
    };

    try {
      const response = await loginTeacher(payload).unwrap();
      ...
      if (response?.accessToken) {
        ...
        setTimeout(() => {
          router.push('/teacher/pages/dashboard');
        }, 1000);
      }
    } catch (error: unknown) {
