...
import { useLoginSchoolAdminMutation } from '@/app/store/api/auth/authApi';
...
export default function LoginPage() {
  ...
  const [loginSchoolAdmin] = useLoginSchoolAdminMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await loginSchoolAdmin({ email, password }).unwrap();
      ...
      if (response?.accessToken) {
        ...
        setTimeout(() => {
          router.push('/school-admin/pages/dashboard');
        }, 1000);
      }
    } catch (error: unknown) {
