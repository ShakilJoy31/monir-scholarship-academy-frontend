...
import { useAcceptClassFeePayMutation, useBulkAcceptClassFeePayMutation, useBulkCancelClassFeePayMutation, useCancelClassFeePayMutation, useCreateClassFeePayMutation, useDeleteClassFeePayMutation, useGetAllClassFeePaysQuery, useGetClassFeePayByIdQuery, useUpdateClassFeePayMutation } from "@/app/store/api/classes/classFeePayApi";
...
interface ClassFeePay {
    id: number;
    branchId: number;
    classFeeAssignId: number;
    status: "Pending" | "Accepted" | "Canceled";
    amount: number;
    note: string | null;
    createdAt: string;
    updatedAt: string;
    classFeeAssign: ClassFeeAssign;
    Payment: Payment[];
    selected?: boolean;
    [key: string]: unknown;
}
...
const ClassFeePayList = () => {
    ...
    const [filteredStudents, setFilteredStudents] = useState<ClassFeePay[]>([]);

    const { data: unpaidStudentResponse } = useGetUnpaidClassFeeAssignsByTeacherQuery(
        selectedStudent?.id,
    ...
    const [currentFeePay, setCurrentFeePay] = useState<ClassFeePay | null>(null);
    ...
    const [createFeePay, { isLoading: isCreatingFeePay }] = useCreateClassFeePayMutation();
    const [updateFeePay, { isLoading: isUpdatingFeePay }] = useUpdateClassFeePayMutation();
    const [acceptFeePay] = useAcceptClassFeePayMutation();
    const [cancelFeePay] = useCancelClassFeePayMutation();
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (confirmAction.action === 'accept') {
                ...
                toastShowing('Fee payment accepted successfully', 'bottom-right', 2000, 'green', 'white');
            } else if (confirmAction.action === 'cancel') {
                await cancelFeePay(confirmAction.feePayId).unwrap();
                toastShowing('Fee payment cancelled successfully', 'bottom-right', 2000, 'green', 'white');
            }
            refetch();
        } catch (err) {
            toast.error(
                (err as { data?: { message?: string } })?.data?.message ||
                `Failed to ${confirmAction.action} fee payment`
            );
        } finally {
    ...
    const handleOpenEditFeePayModal = (feePay: ClassFeePay) => {
        setCurrentFeePay(feePay);

        // Set the student data for the fee pay being edited
        const studentData = feePay.classFeeAssign?.student;
        if (studentData) {
            setSelectedStudent(studentData);

            // Set the month/year selection
            setSelectedMonth({
                id: feePay.classFeeAssignId,
                month: feePay.classFeeAssign.month,
                year: feePay.classFeeAssign.year,
                netPayable: feePay.amount
            });
        }

        setFeePayData({
            classFeeAssignId: feePay.classFeeAssignId,
    ...
    const handleOpenViewFeePayModal = (feePay: ClassFeePay) => {
        setCurrentFeePay(feePay);
    ...
    const handleCreateOrUpdateFeePay = async () => {
        try {
            if (!feePayData.classFeeAssignId || feePayData.amount <= 0) {
                toastShowing('Class fee assignment and valid Amount are required', 'bottom-right', 2000, 'red', 'white');
                return;
            }
            ...
            if (currentFeePay) {
                ...
                toastShowing('Fee payment updated successfully', 'bottom-right', 2000, 'green', 'white');
            } else {
                await createFeePay(modifiedPayload).unwrap();
                toastShowing('Fee payment created successfully', 'bottom-right', 2000, 'green', 'white');
            }
            ...
            toast.error(
                (err as { data?: { message?: string } })?.data?.message ||
                (currentFeePay ? "Failed to update fee payment" : "Failed to create fee payment")
            );
    ...
    const handleDeleteFeePay = async () => {
        await handleDeleteConfirmation(
            ...
            {
                successMessage: "Fee payment deleted successfully",
                errorMessage: "Failed to delete fee payment",
            }
        );
    };
    ...
    const columns = [
        ...
        },
        {
            key: 'sl',
            header: 'SL',
            render: (row: ClassFeePay, index: number) => (page - 1) * rowsPerPage + index + 1
        },
        {
            key: 'student',
            header: 'Student Information',
            render: (row: ClassFeePay) => {
                const student = row.classFeeAssign?.student;
                return (
                    <Box>
        ...
        },
        {
            ...
            header: 'Month/Year',
            render: (row: ClassFeePay) => {
                return `${row.classFeeAssign?.month || 'N/A'} ${row.classFeeAssign?.year || ''}`;
            }
        },
        {
            key: 'amount',
            header: 'Amount',
            render: (row: ClassFeePay) => `৳ ${row.amount.toFixed(2)}/-`
        },
        {
            key: 'status',
            header: 'Status',
        ...
        <Box>
            ...
            {confirmAction.action === 'accept' ? (
                <AcceptModal
                    ...
                    description="Are you sure you want to accept this fee payment? This action cannot be undone."
                ...
                <DeleteConfirmationModal
                    ...
                    description="Are you sure you want to cancel this fee payment? This action cannot be undone."
                    isLoading={isDeleting}
                />
            )}
                        ...
                        >
                            <DialogTitle sx={{
                                ...
                                <Typography variant="h6">
                                    {currentFeePay ? "Edit Fee Payment" : "Add Fee Payment"}
                                </Typography>
                                            ...
                                            <Box sx={{ mb: 2 }}>
                                                <FormControl fullWidth>
                                                    <Autocomplete
                                                        ...
                                                        value={currentStudent || selectedStudent}
                                                        ...
                                                        filterOptions={(options) => options} // Disable default filtering since we're handling it via API
                                                        clearOnBlur={false}
                                                        freeSolo={false}
                                                    />
                                                </FormControl>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                ...
                                                <div>
                                                    ...
                                                    <Typography variant="body1" fontWeight={500}>
                                                        {currentFeePay.classFeeAssign?.student?.name || 'N/A'}
                                                    </Typography>
                                                    <Typography variant="body2" color="text.secondary">
                                                        ID: {currentFeePay.classFeeAssign?.student?.studentUniqueId || 'N/A'} •
                                                        Class: {currentFeePay.classFeeAssign?.student?.class?.name || 'N/A'} •
                                                        Roll: {currentFeePay.classFeeAssign?.student?.classRoll || 'N/A'}
                                                    </Typography>
                                                </div>
                                            </Box>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                ...
                                                <div>
                                                    ...
                                                    <Typography variant="body1">
                                                        {currentFeePay.classFeeAssign?.month || 'N/A'} {currentFeePay.classFeeAssign?.year || ''}
                                                    </Typography>
                                                </div>
                                            </Box>
            ...
            <Paper>
                {isLoading ? (
                    ...
                    >
                        No fee payments found. {searchTerm ? "Try a different search term." : "Create your first fee payment."}
                    </Typography>
                ) : (
                    <>
                        <ReusableTable<ClassFeePay>
                            columns={columns}
            ...
            <DeleteConfirmationModal
                ...
                description="Are you sure you want to delete this fee payment record? This action cannot be undone."
                isLoading={isDeleting}
            />
        </Box>
    );
...
export default ClassFeePayList;