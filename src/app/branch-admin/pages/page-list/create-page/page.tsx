...
import { Box, Typography, Select, MenuItem, InputLabel, FormControl, TextField, CircularProgress, useMediaQuery, useTheme } from "@mui/material";
...
import { useCreatePageMutation, useGetPageByIdQuery, useUpdatePageMutation } from "@/app/store/api/classes/PageApi";
...
const PageForm = () => {
    ...
    const pageId = ''
    const { data: existingPage } = useGetPageByIdQuery(pageId || "", { skip: !pageId });
    ...
    const handleImageUploadBefore = (files: File[], info: object, uploadHandler: (response: { result?: Array<{ url: string; name: string; size: number }>; error?: { message: string } }) => void) => {
        const file = files[0];
        ...
        // Return false to indicate async upload is handled manually
        return false;
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (pageId) {
                await updatePage({ id: Number(pageId), ...pageData }).unwrap();
                toastShowing('Page updated successfully', 'bottom-right', 2000, 'green', 'white');
            } else {
                await createPage(pageData).unwrap();
                toastShowing('Page created successfully', 'bottom-right', 2000, 'green', 'white');
            }
            ...
            toastShowing(
                ...
                (pageId ? "Failed to update page" : "Failed to create page"),
                'bottom-right',
                2000,
                'red',
                'white'
