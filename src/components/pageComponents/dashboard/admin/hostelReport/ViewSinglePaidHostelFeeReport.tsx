...
import { useGetSinglePaidHostelFeeQuery } from "@/app/store/api/allReport/allReportApi";
...
const ViewSinglePaidHostelFeeReport: React.FC<
  ViewSingleUnpaidClassFeeReportProps
> = ({ open, onClose, studentId }) => {
  const { data, isLoading, isError } = useGetSinglePaidHostelFeeQuery(
    studentId,
    {
      skip: !studentId || !open,
    }
   ...
   const handlePrint = () => {
    if (feeData.length > 0) {
      printSingleData(
        feeData[0],
        singleRecordColumns,
        "Student Paid Hostel Fee Details",
        headerInfoGenerator,
        summaryGenerator
      );
    }
  ...
  const handlePDF = () => {
    if (feeData.length > 0) {
      generateSinglePDF(
        ...
        singleRecordColumns,
        "Student Paid Hostel Fee Details",
        `student_paid_hostel_fee_${studentInfo?.studentUniqueId}`,
        headerInfoGenerator,
        summaryGenerator
      );
    }
                  ...
                  <TableBody>
                    {feeData.map((row, i) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>
                          ৳{row?.hostelFee.toLocaleString() || "0"}
                        </TableCell>
...
export default ViewSinglePaidHostelFeeReport;
