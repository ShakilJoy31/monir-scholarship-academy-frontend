...
import { useGetSingleUnpaidHostelFeeQuery } from "@/app/store/api/allReport/allReportApi";
...
const ViewSingleUnpaidHostelFeeReport: React.FC<ViewSingleUnpaidClassFeeReportProps> = ({
  open,
  onClose,
  studentId,
}) => {
  const { data, isLoading, isError } = useGetSingleUnpaidHostelFeeQuery(studentId, {
    skip: !studentId || !open,
  });
  ...
  const studentInfo = feeData[0]?.student;
    ...
    const singleRecordColumns = [
      ...
      },
      {
        key: "hostelFee",
        header: "Hostel Fee",
        render: (row: any) => `৳${row.hostelFee?.toLocaleString() || "0"}`
      },
      {
        key: "discount",
        header: "Discount",
     ...
     const handlePrint = () => {
      if (feeData.length > 0) {
        printSingleData(
          feeData[0],
          singleRecordColumns,
          "Student Unpaid Hostel Fee Details",
          headerInfoGenerator,
          summaryGenerator
        );
      }
    ...
    const handlePDF = () => {
      if (feeData.length > 0) {
        generateSinglePDF(
          feeData[0],
          singleRecordColumns,
          "Student UnP=paid Hostel Fee Details",
          `student_unpaid_hostel_fee_${studentInfo?.studentUniqueId}`,
          headerInfoGenerator,
          summaryGenerator
        );
      }
                  ...
                  <TableBody>
                    {feeData.map((row, i) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>৳{row?.hostelFee.toLocaleString() || "0"}</TableCell>
...
export default ViewSingleUnpaidHostelFeeReport;