...
import { useGetUnpaidExamFeeQuery } from "@/app/store/api/allReport/allReportApi";
import ReusableTable from "@/components/shared/reusable-component/ReusableTable";
import PaginationComponent from "@/components/shared/reusable-component/PaginationComponent";
import ViewSingleUnpaidExamFeeReport from "./ViewSingleUnpaidExamFeeReport";
...
const UnpaidExamFeeReport = () => {
  ...
  const {
    ...
    isError,
    refetch,
  } = useGetUnpaidExamFeeQuery(selectedClassId, {
   ...
   const { printData } = usePrint<StudentFeeData>();
      ...
      const handlePrint = () => {
        ...
        const printColumns = columns
          ...
          .map(col => ({
            ...col,
            header: typeof col.header === 'string' ? col.header : (typeof col.header === 'object' && 'props' in col.header && col.header.props?.children ? col.header.props.children : col.key)
          }));
    
        printData(selectedData, printColumns, 'Unpaid Exam Fee Report');
      };
        ...
        const pdfColumns = columns
          ...
          .map(col => ({
            ...
            header: typeof col.header === 'string'
              ? col.header
              : (typeof col.header === 'object' && 'props' in col.header && col.header.props?.children
                  ? col.header.props.children
                  : col.key),
            render: col.render
          }));
    
        await generatePDF(selectedData, pdfColumns, 'Unpaid Exam Fee Report', 'unpaid_exam_fee_report');
      };
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" component="h1" gutterBottom>
          Unpaid Exam Fee Report
        </Typography>
        <Typography variant="body1" color="textSecondary">
          View and manage unpaid exam fees for students
        </Typography>
      </Box>
      ...
      <Paper sx={{ p: 3, mb: 3, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Box sx={{ display: "flex", gap: 2, alignItems: "flex-end" }}>
         ...
         <div className="flex gap-4">
                                        <SubmitButton onClick={handlePrint} disabled={selectedStudents.length === 0}>
                            <span className="font-medium">Print</span>
                          </SubmitButton>
                        
                                         <CancelButton onClick={handlePDF} disabled={selectedStudents.length === 0}>
      ...
      <Paper>
        {isLoading ? (
          ...
          <Typography
            ...
            sx={{ p: 4, textAlign: "center" }}
          >
            No unpaid fees found for the selected exams
          </Typography>
        ) : (
          <>
...
export default UnpaidExamFeeReport;