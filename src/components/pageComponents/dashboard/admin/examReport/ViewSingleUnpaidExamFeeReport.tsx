...
import { useGetSingleUnpaidExamFeeQuery } from "@/app/store/api/allReport/allReportApi";
...
const ViewSingleUnpaidExamFeeReport: React.FC<ViewSingleUnpaidClassFeeReportProps> = ({
  open,
  onClose,
  studentId,
}) => {
  const { data, isLoading, isError } = useGetSingleUnpaidExamFeeQuery(studentId, {
    skip: !studentId || !open,
  });
   ...
   const handlePrint = () => {
    if (feeData.length > 0) {
      printSingleData(
        feeData[0],
        singleRecordColumns,
        "Student Unpaid Fee Exam Details",
        headerInfoGenerator,
        summaryGenerator
      );
    }
  ...
  const handlePDF = () => {
    if (feeData.length > 0) {
      generateSinglePDF(
        ...
        singleRecordColumns,
        "Student Unpaid Fee Exam Details",
        `student_unpaid_exam_fee_${studentInfo?.studentUniqueId}`,
        headerInfoGenerator,
        summaryGenerator
      );
    }
...
export default ViewSingleUnpaidExamFeeReport;