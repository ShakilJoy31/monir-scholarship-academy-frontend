...
import { useGetPaidExamFeeQuery } from "@/app/store/api/allReport/allReportApi";
import ReusableTable from "@/components/shared/reusable-component/ReusableTable";
import PaginationComponent from "@/components/shared/reusable-component/PaginationComponent";
import ViewSinglePaidExamFeeReport from "./ViewSinglePaidExamFeeReport";
...
const PaidExamFeeReport = () => {
  ...
  const {
    ...
    isError,
    refetch,
  } = useGetPaidExamFeeQuery(selectedClassId, {
  ...
  const handlePrint = () => {
    ...
    printData(selectedData, printColumns, "Paid Exam Fee Report");
  };
  ...
  const handlePDF = async () => {
    ...
    await generatePDF(
      selectedData,
      pdfColumns,
      "Paid Exam Fee Report",
      "paid_Exam_fee_report"
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: StudentFeeData) =>
        (
          (row.totalUnpaid as number) + (row.totalDiscount as number)
        ).toLocaleString(),
    },
    {
      key: "paid",
      header: "Paid",
      render: (row: StudentFeeData) =>
        (row.totalPay as number).toLocaleString(),
    },
    {
      key: "totalDue",
      header: "Total Due",
      render: (row: StudentFeeData) =>
        (row.totalUnpaid as number).toLocaleString(),
    },
    {
      key: "parent",
      header: "Parent",
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body1" color="textSecondary">
          View and manage paid exam fees for students
        </Typography>
      </Box>
      ...
      <Paper>
        {isLoading ? (
          ...
          >
            Please select a exam and click Search to view paid fees
          </Typography>
        ) : students.length === 0 ? (
          <Typography
            variant="body1"
            ...
            sx={{ p: 4, textAlign: "center" }}
          >
            No paid fees found for the selected exams
          </Typography>
        ) : (
          <>
...
export default PaidExamFeeReport;
