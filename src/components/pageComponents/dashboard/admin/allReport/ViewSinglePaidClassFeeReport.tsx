...
import { useGetSinglePaidClassFeeQuery } from "@/app/store/api/allReport/allReportApi";
...
const ViewSinglePaidClassFeeReport: React.FC<
  ViewSingleUnpaidClassFeeReportProps
> = ({ open, onClose, studentId }) => {
  const { data, isLoading, isError } = useGetSinglePaidClassFeeQuery(
    studentId,
    {
      skip: !studentId || !open,
    }
    ...
    const handlePDF = () => {
      if (feeData.length > 0) {
        generateSinglePDF(
          ...
          singleRecordColumns,
          "Student Paid Fee Details",
          `student_paid_fee_${studentInfo?.studentUniqueId}`,
          headerInfoGenerator,
          summaryGenerator
        );
      }
...
export default ViewSinglePaidClassFeeReport;
