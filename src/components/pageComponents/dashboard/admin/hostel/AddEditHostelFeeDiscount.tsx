...
import {
  HostelFeeDiscountFormValues,
  HostelFeeDiscountSchema,
} from "@/app/super-admin/schemas/hostel/hostelFeeDiscountSchema";
...
interface AddEditHostelFeeDiscountProps {
  open: boolean;
  onClose: () => void;
  onSubmit: SubmitHandler<HostelFeeDiscountFormValues>;
  currentData: { id: number | null; data: HostelFeeDiscountFormValues & { studentUniqueId?: string } } | null;
  isLoading: boolean;
  error: string | null;
  onErrorDismiss: () => void;
  title: string;
...
const AddEditHostelFeeDiscount = ({
  open,
  onClose,
  onSubmit,
  currentData,
  ...
  onErrorDismiss,
  title,
}: AddEditHostelFeeDiscountProps) => {
  const methods = useForm<HostelFeeDiscountFormValues>({
    resolver: zodResolver(HostelFeeDiscountSchema),
    defaultValues: {
      studentId: 0,
      discountType: "Fixed",
      discount: 0,
  ...
  const handleFormSubmit = async (data: HostelFeeDiscountFormValues) => {
    try {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      {watch("discountType") === "Percentage" ? "%" : "BTD"}
                    </InputAdornment>
                  ),
                }}
              />
      ...
      <DialogActions sx={{ p: 2 }}>
        ...
        <SubmitButton
          type="submit"
          disabled={isLoading || isUploading || !currentStudent}
...
export default AddEditHostelFeeDiscount;