...
import { useGetSinglePaidTeacherFeeQuery } from "@/app/store/api/allReport/allReportApi";
...
const ViewSinglePaidTeacherFeeReport: React.FC<
  ViewSingleUnpaidClassFeeReportProps
> = ({ open, onClose, studentId }) => {
  const { data, isLoading, isError } = useGetSinglePaidTeacherFeeQuery(
    studentId,
    {
      skip: !studentId || !open,
    }
  ...
  const summaryGenerator = (data: any) => `
    <strong>Total Paid Amount:</strong> ৳${feeData
  ...
  const handlePrint = () => {
    if (feeData.length > 0) {
      printSingleData(
        feeData[0],
        singleRecordColumns,
        "Teacher Paid Salary Fee Details",
        headerInfoGenerator,
        summaryGenerator
      );
    }
  ...
  const handlePDF = () => {
    if (feeData.length > 0) {
      generateSinglePDF(
        ...
        singleRecordColumns,
        "Teacher Paid Salary Fee Details",
        `teacher_paid_salary_fee_${studentInfo?.teacherUniqueId}`,
        headerInfoGenerator,
        summaryGenerator
      );
    }
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
      ...
      <DialogContent sx={{ p: 3 }}>
        ...
        {isLoading ? (
          ...
          <Typography textAlign="center" sx={{ my: 3 }}>
            No paid salary data found for this student.
          </Typography>
        ) : (
          <>
            {/* Teacher Info */}
...
export default ViewSinglePaidTeacherFeeReport;
